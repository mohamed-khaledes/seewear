...
import { exportPaymentsCsv } from "@/features/dashboard/server";
...
export async function GET(request: NextRequest) {
  ...
  const csv = await exportPaymentsCsv({ from, to });

  const stamp = [from, to].filter(Boolean).join("_to_") || "all";
  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="seewear-payments-${stamp}.csv"`,
